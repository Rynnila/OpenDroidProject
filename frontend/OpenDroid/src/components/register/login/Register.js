...
function Register() {
  ...
  const validate = () => {
    ...
    if (!formData.password) {
      ...
      newErrors.password = 'Senha deve ter 3 caracteres';
    }
    ...
    if (validate()) {
      try {
        ...
        if (!response) {
          ...
          return; // Para o registro se o email já existir
        }

        AlertsService.swalAlert('success', 'Conta criada com sucesso!', false, 5000);
        navigate('/opendroid/login');
      } catch (error) {
    ...
    <div className="register-container">
      <form className="form" onSubmit={handleSubmit}>
        <div className="register-box">
          <img className="logo" src="assets/imgs/logo-opendroid-vertical.png" alt="logo-opendroid" />
          <div className="input-container">
            <i className="icon" name="mail"></i>
            <input
              className="input"
              type="email"
              placeholder="Email"
          ...
          <div className="login-container">
            ...
            <span className="login" onClick={() => navigate('/login')}>Entrar!</span>
          </div>
        </div>
      </form>
    </div>
